...
**Actualización 2026-07-21 — diagnóstico de telemetría fallada en el servidor real + fix de incidencias:**
- El usuario reportó que tras desplegar en el servidor real (con HTTPS 8443 ya configurado), la telemetría seguía sin funcionar, y que además un Conductor no podía reportar una incidencia porque su propio turno no aparecía en el selector.
- **Bug real encontrado y corregido:** `src/modulos/incidencias/index.php` comparaba `t.dni_conductor` (BD) contra `$_SESSION['nombre_usuario']` (usuario de login) en vez de `$_SESSION['dni']`, en dos lugares — el filtro de la lista de incidencias del Conductor (línea ~14) y el `SELECT` que llena el `<select>` de turno al reportar una incidencia nueva (línea ~137). Como `nombre_usuario` nunca es igual a un `dni`, el Conductor jamás veía su propio turno ni sus propias incidencias. `turnos/index.php` no tiene este bug — ya usaba `$_SESSION['dni']` correctamente. Corregido y confirmado en vivo desde el celular del usuario.
- **Infraestructura agregada para poder reproducir el bloqueo de geolocalización localmente** (Docker está en `localhost`, exento del requisito de "contexto seguro", así que no reproducía el problema del servidor por sí solo): se habilitó `mod_ssl` en el `Dockerfile`, se agregó un vhost (`docker/ssl.conf`, sí versionado) sirviendo HTTPS en el puerto 443 del contenedor, mapeado a `8443` en `docker-compose.yml` (mismo puerto que usa el servidor real). El certificado autofirmado (`subjectAltName=IP:<IP-LAN-del-dev>`, generado con el mismo comando `openssl` del README) vive en `docker-ssl/` — **gitignoreado**, porque es específico de la IP de LAN de quien lo genera y no debe commitearse; cada dev que quiera repetir esta prueba debe regenerarlo con su propia IP (ver README, sección "HTTPS para telemetría móvil — probarlo en local antes de desplegar"). También hizo falta una regla de Firewall de Windows (perfil Privado, puerto 8443 entrante) para que el celular pudiera alcanzar el Docker del host — no hay ninguna regla de Docker/8080/8443 por defecto.
- **Resultado del diagnóstico: no había ningún bug de código en la telemetría.** Se confirmó sirviendo `turnos/index.php` vía `curl` logueado como un Conductor real con turno `Activo` (usuario `pedro12`, creado por el usuario durante esta sesión) que el bloque `navigator.geolocation.watchPosition(...)` se renderiza correctamente con el CSRF token correcto. La causa real era ambiental: **el servicio de Ubicación del celular (Android) estaba apagado**, así que Chrome nunca disparaba el permiso pese a que el HTTPS del lado servidor estaba bien. Una vez el usuario activó Ubicación en el sistema, la telemetría empezó a llegar en vivo a la tabla `telemetria` (confirmado con filas reales de `pedro12`/CAM-002 cada ~15s).
- **Para repetir este diagnóstico en el servidor real**, si vuelve a fallar: revisar primero Ubicación del sistema + permiso de Chrome/Safari a nivel app + `chrome://settings/content/location` (o equivalente iOS) antes de sospechar del código — el flujo servidor (`registrar_ubicacion.php` + JS de `turnos/index.php`) ya está verificado funcionando end-to-end.
- **Ajustes de interfaz pedidos por el usuario durante la misma prueba:** `telemetria/index.php` mezclaba en una sola tabla cruda las posiciones de todos los conductores activos, ordenadas solo por fecha — con 2+ conductores a la vez, la última posición de un vehículo se perdía entre las actualizaciones de los demás. Se agregó un panel nuevo "Última ubicación por vehículo" (una fila fija por camión, la más reciente) antes del historial crudo (renombrado "Historial completo (log crudo)"), con 24px de separación entre ambos paneles (mismo patrón `style="margin-bottom:24px;"` que ya usa `mantenimiento/index.php` para sus dos tablas apiladas). También se agregó un buscador en vivo (`data-buscar-tabla="#idTabla"` en `global.js`, sin librerías) en `usuarios`, `flota` y `conductores` — filtra filas de la tabla mientras se escribe, sin recargar la página; el mismo atributo es reutilizable en cualquier otro módulo con una tabla.
- **Limitación real descubierta al probar en el celular, documentada (no resuelta a propósito):** el envío de GPS solo funciona con la pestaña de `turnos/index.php` abierta y en primer plano — si el conductor cambia de app o bloquea la pantalla, Android/iOS pausan el JS en segundo plano y el envío se corta. No es arreglable con un ajuste de código (requeriría una app nativa); documentado como indicación operativa en `README.md` (sección "Telemetría móvil — la pantalla tiene que quedar encendida") y en `MANUAL_SISTEMA.md` sección 9. Se le mencionó al usuario la Screen Wake Lock API como mitigación parcial (evita que la pantalla se apague sola, no resuelve cambiar de app) — no implementada, pendiente a pedido explícito del usuario de solo documentar por ahora.
